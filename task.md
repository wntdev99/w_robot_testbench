...
26. **P2 저작 UI 완료** (`c9667eb`~`0ff23ce`) — introspect API(topics/services/typeFields 재귀) + 프로젝트 CRUD(user 쓰기·create/update/delete/duplicate) + 레이아웃 빌더(위젯 팔레트·편집·저장) + 위젯 동적폼(임의 타입 재귀). **202 검증**: typeFields Twist 재귀·CRUD·builtin→user 복제 전부 ✅. 빌드 통과
...
- [x] **텔레옵 패드 위젯**(control.teleop) — main 패드 일반화: 화면패드/게임패드/키보드 3소스, 데드맨·스프링복귀·REP-103·속도상한(max_lin/max_yaw), live 게이트. builtin teleop.yaml 적용
...
- [x] P2 202 통합 검증 ✅ — introspect(typeFields Twist 재귀 트리)·createProject·update(위젯 추가)·목록(builtin+user)·duplicate(builtin→user, cloned_from)·delete 전부 동작. **P2 완료**
