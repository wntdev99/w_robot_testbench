...
@router.get("/frame")
async def frame(topic: str, request: Request):
    cam = request.app.state.ctx.cam
    if not cam.available():
        raise HTTPException(503, "카메라 변환 의존성(cv2/cv_bridge) 없음")
    loop = asyncio.get_event_loop()
    # 구독 시작 + cv 변환/JPEG 인코딩은 이벤트루프/spin 을 막지 않도록 스레드풀에서
    status, data = await loop.run_in_executor(None, cam.request_frame, topic, time.time())
    if status == "limit":
        raise HTTPException(429, f"동시 카메라 스트리밍 제한({cam.max_concurrent}대) 초과")
    if not data:
        return Response(status_code=204)        # 구독했으나 아직 프레임 없음
